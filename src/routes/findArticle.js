...
async function findArticle(req, res) {
    // the task had mentioned to search by title or author. but there is no api to search based on author.
    let { title } = req.query;
    if (!title)
        return res.status(400).json({ error: 'Bad Request', message: 'title is a required param' });
